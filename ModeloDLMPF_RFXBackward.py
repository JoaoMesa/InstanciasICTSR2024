...
def main():
    ...
    model.parameters.emphasis.mip = 1  # Prioriza a rapidez na solução
    ...
    contador = 0
    for bloco_inicio in range(T - 1, 0, -tamanho_do_bloco):
        contador+=1
        bloco_fim = max(bloco_inicio - tamanho_do_bloco + 1, 1) 
        indices_binario = list(range(bloco_fim, bloco_inicio + 1)) 
        ...
        #print("Solução para o bloco ", indices_binario, ":", nova_solucao)

        print("Períodos Binárias/Inteiros no Bloco Atual: ", indices_binario)
            #print("Nova Solução: ", nova_solucao)
            
            # Fixar os valores após N blocos (controle mais lento)
        if contador % 2 == 0:  
