...
def main():
    ...
    #model.set_time_limit(4*4*60)
    
    
    #============================================================================================================================================
   
    # Aceita a primeira solução factível
    
    #============================================================================================================================================

    model.parameters.mip.tolerances.mipgap = 0.01  # Permite um gap de 1% para soluções rápidas
    model.parameters.threads = 4  # Utiliza múltiplos núcleos, caso disponível
    ...
    def fixar_variaveis(fixar_indices, FOsolucao):
        for i in CONJ_P_itens:
            for k in CONJ_L_lin:
                for t in range(1,T):
                    if t in fixar_indices: 
                        x[i,k,t].lb = x[i,k,t].ub = FOsolucao.get_value(x[i,k,t])
                        y[i,k,t].lb = y[i,k,t].ub = FOsolucao.get_value(y[i,k,t])
                        w[i,k,t].lb = w[i,k,t].ub = FOsolucao.get_value(w[i,k,t])
                        z[i,k,t].lb = z[i,k,t].ub = FOsolucao.get_value(z[i,k,t])
